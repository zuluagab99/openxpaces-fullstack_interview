...
from app.schemas.schemas import (
    RawDealInput, ImportResult, ImportError,
    DealResponse, DealsListResponse,
...
router = APIRouter(prefix="/deals", tags=["deals"], dependencies=[Depends(require_api_key)])
...
def _run_import(job_id: str, records: list[dict]):
    ...
    try:
        ...
        for index, raw in enumerate(records):
            ...
            if not result.ok:
                for e in result.errors:
                    errors.append(ImportError(index=e.index, field=e.field, raw=e.raw, reason=e.reason))
                continue
            ...
            # upsert tenant
            tenant = db.query(Tenant).filter(func.lower(Tenant.name) == deal.tenant_name.lower()).first()
            if not tenant:
            ...
            if not property_:
                property_ = Property(street=deal.street, city=deal.city, state=deal.state, zip=deal.zip)
                db.add(property_)
                db.flush()

            flags = detect_anomalies(deal)
        ...
        db.commit()
        cache_invalidate("market:")  # bust analytics cache after new data

        job_store.set_done(job_id, ImportResult(
...
# ---------------------------------------------------------------------------
# POST /deals/import  — kicks off background job, returns immediately
# ---------------------------------------------------------------------------

@router.post("/import", response_model=JobResponse, status_code=202)
def import_deals(
    records: list[RawDealInput],
    background_tasks: BackgroundTasks,
):
    job_id = job_store.create_job()
    raw_records = [r.model_dump() for r in records]
    background_tasks.add_task(_run_import, job_id, raw_records)
    return JobResponse(
        job_id=job_id,
        status="pending",
        message=f"Import job queued for {len(records)} records.",
    )


# ---------------------------------------------------------------------------
# GET /deals/import/status/{job_id}  — poll for job result
# ---------------------------------------------------------------------------
...
def import_status(job_id: str):
    ...
    return JobStatusResponse(
        job_id=job_id,
        status=job["status"],
        result=ImportResult(**job["result"]) if job["result"] else None,
        error=job["error"],
    )
...
def list_deals(
    ...
    if city:        q = q.filter(func.lower(Property.city)  == city.lower())
    if state:       q = q.filter(func.lower(Property.state) == state.lower())
    if lease_type:  q = q.filter(Deal.lease_type == LeaseType(lease_type.upper()))
    if sqft_min is not None: q = q.filter(Deal.size_sqft >= sqft_min)
    if sqft_max is not None: q = q.filter(Deal.size_sqft <= sqft_max)
    if rent_min is not None: q = q.filter(Deal.rent_psf >= rent_min)
    if rent_max is not None: q = q.filter(Deal.rent_psf <= rent_max)
    if search:
        term = f"%{search.lower()}%"
        q = q.filter(or_(
    ...
    sort_col = {"rent_psf": Deal.rent_psf, "size_sqft": Deal.size_sqft, "created_at": Deal.created_at}[sort_by]
